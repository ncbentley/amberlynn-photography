# Allura Typography Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Apply Allura to display typography, retain Young Serif for homepage category titles and subtitles, and retain Karla for body and interface copy.

**Architecture:** Use the regular Allura Fontsource package for `--font-display` and retain Young Serif through a separate `--font-homepage-display` token. Homepage category titles and subtitles explicitly use the homepage token; body and interface text continue to inherit Karla.

**Tech Stack:** Astro 7, CSS custom properties, Fontsource, npm

## Global Constraints

- Bundle Allura locally with no runtime font CDN request.
...
- Keep branding such as the Amberlynn wordmark in Allura, including on the homepage.
...
- Produces: locally bundled Allura exposed through `var(--font-display)`

- [ ] **Step 1: Record pre-change assertions**

Run `rg -n "@fontsource/young-serif|'Young Serif'|@fontsource/allura|'Allura'" package.json package-lock.json src/layouts/Base.astro src/styles/global.css`.

Expected: Young Serif appears in the dependency, lockfile, import, and display token; Allura does not.

- [ ] **Step 2: Replace the package**

Run `npm uninstall @fontsource/young-serif` and then `npm install @fontsource/allura`.

Expected: the manifest and lockfile contain Allura and no Young Serif.
...
```css
--font-display: 'Allura', cursive, Georgia, serif;
```
...
Expected: Allura appears in the dependency, lockfile, import, and token; Young Serif is absent.
...
- Produces: Allura on the hero tagline, category subtitle, and homepage CTA line
...
**Interfaces:**
- Consumes: Allura through `var(--font-display)`
...
Expected: the assertions fail the revised requirement because the homepage uses Allura.

- [ ] **Step 2: Restore the local Young Serif package and import**

Run `npm install @fontsource/young-serif`. Add `import '@fontsource/young-serif';` beside the Allura import in `src/layouts/Base.astro`.
...
Use `font-family: var(--font-homepage-display)` only for `.cat-name` and `.cat-blurb` in `src/pages/index.astro`. Keep `.tagline`, `.intro h1`, `.cta-line`, and the navigation and footer wordmarks on Allura.
...
Run `rg -n "Allura|allura" dist src package.json package-lock.json`.

Expected: Allura and Young Serif both appear in source/build output and dependencies.
...
Expected: homepage category titles and subtitles use Young Serif; other headings and branding use Allura; body copy and controls remain Karla; no display text clips or overlaps.
...
Stage the dependency, import, token, subtitle style, and plan files. Commit with `git commit -m "apply Allura display typography"`.
